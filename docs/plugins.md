...
内置 `plugin-dev` 向 ChatGPT 提供插件规范、创建、预检、安装、内置可选能力安装、已安装插件管理和宿主更新状态等工具。ChatGPT 可以继续调用工作区读取、写入、编辑和命令工具完成代码，不需要另开编码代理；代理只在用户主动选择或任务明显适合并行时使用。
...
4. 用户要求安装并使用后，ChatGPT 调用 `plugin-dev__install`，显式给出 grants，并在用户要求立即启用时设置 `enable:true`。整条操作仍经过本机批准，不要求用户自己运行终端命令。
5. 已随宿主发布的可选能力使用 `plugin-dev__install_builtin`；已经安装的外部插件和 Computer Use 使用 `plugin-dev__manage` 调整 grants、配置与启停。核心宿主插件不能由这个 MCP 入口修改。

## Capyra 宿主自身更新

普通业务能力应优先实现成外部插件，ChatGPT 可以在同一对话中开发、预检、安装和启用。只有插件契约确实不足、必须修改 Capyra 宿主时，才使用宿主更新流程：

1. ChatGPT 使用工作区和命令工具完成源码修改、构建与 `npm pack`。
2. 读取生成 `.tgz` 的 SHA-256 和版本。
3. 调用 `plugin-dev__host_update`，提交工作区相对包路径、精确 SHA-256 和可选版本。
4. 本机批准卡显示包、摘要、版本、短暂断连和回滚行为；该工具始终逐次确认。
5. 独立更新器把获批包复制到私有状态目录，备份当前全局安装，干净停止当前实例，再安装并按原命令重启。
6. 新实例必须发布自己的 runtime lock 才算成功；安装或启动失败会恢复旧包并重启。固定 MCP 地址和用户配置不变。
7. 原对话在连接恢复后调用 `plugin-dev__host_update_status`，重新发现工具并继续任务。

宿主更新只支持 npm 全局安装的 Capyra。源码开发实例继续直接运行构建结果；更新工具不读取 npm registry，也不接受工作区外的包、软链接、硬链接、超过 100 MB 的归档或摘要不一致的文件。

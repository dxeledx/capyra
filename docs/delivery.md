...
```sh
npm install -g /完整路径/capyra-0.5.0.tgz
cd /你的工作区
capyra init
capyra start --open
```
...
固定入口的 DCR `client_id` 会跨重启长期保留，授权有效期为 30 天；授权到期后 ChatGPT 可以用原客户端身份重新授权，不需要删除并重新添加 Capyra。0.5.0 修复了旧版本在授权过期后从内存清理 `client_id`，导致 reconnect 返回 `invalid_client` 的问题。

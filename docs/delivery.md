...
```sh
npm install -g /完整路径/capyra-0.5.2.tgz
cd /你的工作区
capyra init
capyra start --open
```
...
固定入口的 DCR `client_id` 会跨重启长期保留，授权有效期为 30 天；授权到期后 ChatGPT 可以用原客户端身份重新授权，不需要删除并重新添加 Capyra。0.5.2 同时兼容 ChatGPT 为现有开发者应用轮换 `client_id` 的情况：只有请求中的回调地址精确匹配本机已登记的 ChatGPT 专属回调，才复制公开 DCR 元数据；不会自动产生 grant、token 或绕过本机 OAuth 批准。
